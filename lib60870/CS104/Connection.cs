...
namespace lib60870.CS104
{
    ...
    public enum ConnectionEvent
    {
        ...
        /// <summary>
        /// Подключение потеряно, ожидание восстановления
        /// </summary>
        LOST = 5
    ...
    public class Connection : Master
    {
        ...
        // подписка на событие осущетвляется в PageMikroSRZ
        // при возникновении события осуществляется попытка
        //  переподключения к серверу

        public event ReConnectDelegate ConLost;
        ...
        private void HandleConnection()
        {
            ...
            // логика для организации реконнекта
            // если условие истино, то возникает
            // событие ConLost и осуществляется попытка реконнекта
            // иначе - соединение было закрыто пользователем
            // и реконнект не требуется

            // !!!! в некоторых ситуациях условие может быть истино
            // и при закрытии соединения пользователем
            // для избежания реконнекта в этом случае 
            // осуществляется проверка состояние кнопки отключения 
            // соединения в PageMikroSRZ !!!!

            //if (!timeoutT2Triggered && !socketError)
            //{
            //    if (connectionHandler != null)
            //        connectionHandler(connectionHandlerParameter, ConnectionEvent._LOST);
            //    //ConLost();
            //}
        }
